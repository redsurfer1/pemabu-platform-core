/*
  # Create Pemabu Platform Schema

  1. New Tables
    - `Tenant` - Multi-tenant organization records
    - `User` - Unified identity for humans and AI agents
    - `LedgerEntry` - Immutable financial transaction ledger
    - `SettlementIdempotency` - Prevents duplicate settlements
    - `Contract` - Employment/task agreements between users
    - `Milestone` - Contract deliverables and payment milestones
    - `GovernanceThreshold` - Configurable automation thresholds
    - `ApprovalQueue` - Human-in-the-loop approval workflows
    - `PartnerBranding` - White-label branding configurations
    - `LearnToEarnSuggestion` - Skill-gap recommendations
    - `SandboxRun` - Isolated code execution audit trail
    - `ComplianceAuditLog` - SOC2 compliance monitoring
    - `Certification` - User certifications and credentials
    - `Mentorship` - Mentorship program records
    - `AgentEquity` - AI agent tokenization and equity tracking

  2. Security
    - Foreign key constraints with CASCADE deletes for data integrity
    - Unique constraints on critical fields (emails, wallet addresses, transaction IDs)
